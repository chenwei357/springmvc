...
<script>
	...
	$(function(){
		...
		$("#password").blur(function(){
			...
			if(password.trim() == ""){
				...
				if(!patten.test(password)){
					$("#plabel").css("color","red");
					$("#plabel").html("&nbsp;&nbsp;只能输入6-20个字母、数字、下划线!");
				}else{
					$("#plabel").css("color","green");
					$("#plabel").html("&nbsp;&nbsp;密码可用!");
				}
			}
		});
		...
		$("#repassword").blur(function(){
			...
			if(repassword == ""){
				$("#replabel").css("color","red");
				$("#replabel").html("&nbsp;&nbsp;确认密码不能为空!");
			}else {
				if (repassword != password) {
					$("#replabel").css("color", "red");
					$("#replabel").html("&nbsp;&nbsp;两次密码不一致!");
				}else{
					$("#replabel").css("color", "green");
					$("#replabel").html("&nbsp;&nbsp;两次密码一致!");
				}
			}
		});
		//邮箱校验
		...
		$("#checkImg").click(function(){
			var src = $("#checkImg").attr("src");
			$("#checkImg").attr("src",src + "?" + new Date().getTime());
		});
	})
</script>
</head>
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm" action="${pageContext.request.contextPath}/user/register" method="post" novalidate="novalidate">
						<table>
							<tbody>
								<tr>
									<th><span class="requiredField">*</span>用户名:</th>
									<td><input type="text" id="username" name="username"
										class="text" maxlength="20" value="${form.username}" autofocus="autofocus" />
								...
								<tr>
									...
									<td><input type="password" id="password" name="password"
										class="text" maxlength="20" value="${form.password}" autocomplete="off" />
										<label id="plabel"></label></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>确认密码:</th>
									<td><input type="password" id="repassword" name="password2" class="text"
										maxlength="20" value="${form.password2}" autocomplete="off" />
										<label id="replabel"></label>
										<span style="color:red;">${form.errors.password2}</span>
										</td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>E-mail:</th>
									<td><input type="text" id="email" name="email"
										class="text" value="${form.email}" maxlength="200" />
								...
								<tr>
									...
									<td><span class="fieldSet"> <input type="text"
											...
											src="${pageContext.request.contextPath}/checkImage"
											title="点击更换验证码" />
											<span style="color:red;">${form.errors.checkcode}</span>
									</span></td>
								</tr>
								<tr>
									<th>&nbsp;</th>
						...
						<div class="login">
							...
							<dl>
								<dt>已经拥有账号了？</dt>
								<dd>
									立即登录即可体验在线购物！ <a href="${pageContext.request.contextPath}/user/loginPage">立即登录</a>
								</dd>
							</dl>
						</div>
					</form>
